...
def load_mesh_files(in_vtk):
    ...
    # in_vtk = "../Mesh_Files/Hormozblocks3d.vtk" # Ronak
    txtopen = open(in_vtk, 'r')
    all_vtk_lines = txtopen.readlines()
    txtopen.close()
    skp_hdr_ln_no = 3  # Line no. 4
    # This line contains information about the number of nodes
    n_pts_ln_no = skp_hdr_ln_no + 1
    # The number of nodes in the mesh
    npts = int(all_vtk_lines[n_pts_ln_no].split()[1])
    ...
    for i_pt in range(0, npts):
        ...
        ndes[i_pt, :] = list(
            map(float, all_vtk_lines[n_pts_ln_no+1+i_pt].split()[0:]))

    # This line contains information about the number of tetrahedral cells.
    cells_ln_no = n_pts_ln_no + 1 + npts + 1
    # Get the value of the number of cells (tetrahedra) in the mesh
    ncells = int(all_vtk_lines[cells_ln_no].split()[1])

    tet_ndes = np.zeros((ncells, 4), dtype='int')

    for i_cell in range(0, ncells):
        # Read in the node indices (starting from 0) for individual tetrahedron
        tet_ndes[i_cell, :] = list(
            map(int, all_vtk_lines[cells_ln_no+1+i_cell].split()[1:]))
...
def bary_coord(rxpts, n_a, n_k, n_l, n_m, n_n, ndes):
    ...
    d0 = np.linalg.det([[x1, y1, z1, 1.], [x2, y2, z2, 1.],
                       [x3, y3, z3, 1.], [x4, y4, z4, 1.]])
    if n_a == n_k:
        di = np.linalg.det([[x, y, z, 1.], [x2, y2, z2, 1.], [
                           x3, y3, z3, 1.], [x4, y4, z4, 1.]])
        return di/d0
    elif n_a == n_l:
        di = np.linalg.det([[x1, y1, z1, 1.], [x, y, z, 1.], [
                           x3, y3, z3, 1.], [x4, y4, z4, 1.]])
        return di/d0
    elif n_a == n_m:
        di = np.linalg.det([[x1, y1, z1, 1.], [x2, y2, z2, 1.], [
                           x, y, z, 1.], [x4, y4, z4, 1.]])
        return di/d0
    elif n_a == n_n:
        di = np.linalg.det([[x1, y1, z1, 1.], [x2, y2, z2, 1.], [
        ...
        raise ValueError('node ', n_a, ' is not represented by',
                         n_k, n_l, n_m, n_n, '. Please check.')
...
def crl_dot_crl(g_na, g_nb, g_nc, g_nd, det_rk):
    ...
    crl_i = crl_edge(g_na, g_nb)
    crl_j = crl_edge(g_nc, g_nd)
...
def inv_mat3x3(r_k, det_rk):
    ...
    rk_inv = np.zeros((3, 3))
    if det_rk == 0.0:
        raise ValueError('Determinant ', det_rk,
                         '. Matrix is singular, please check.')
    else:
...
def create_sparse_initial_FE_matrix_vector(itet):
    ...
    for iedg in range(6):
        if (in_edge_tet[itet, iedg] > -1):
            ...
            cici, eiei = compute_crlcrl_edgedg(
                ei, ei, nk, nl, nm, nn, gnk, gnl, gnm, gnn, detrk)
            ...
            for jedg in range(iedg+1, 6):
                if (in_edge_tet[itet, jedg] > -1):
                    ...
                    cicj, eiej = compute_crlcrl_edgedg(
                        ei, ej, nk, nl, nm, nn, gnk, gnl, gnm, gnn, detrk)
        ...
        # Filling up RHS
            ...
            if w1 > -1:
                inteiwk = int_edge_wire_seg(
                    ei, w1, nk, nl, nm, nn, gnk, gnl, gnm, gnn)
            ...
            if w2 > -1:
                inteiwk = int_edge_wire_seg(
                    ei, w2, nk, nl, nm, nn, gnk, gnl, gnm, gnn)
                rhs_fillterm = Io*inteiwk*le[ei_ind]*mu0
                elid_rhs = ei_ind
                elidRHS.append(('RHS', elid_rhs, rhs_fillterm))
            ...
            if w3 > -1:
                inteiwk = int_edge_wire_seg(
                    ei, w3, nk, nl, nm, nn, gnk, gnl, gnm, gnn)
                rhs_fillterm = Io*inteiwk*le[ei_ind]*mu0
                elid_rhs = ei_ind
                elidRHS.append(('RHS', elid_rhs, rhs_fillterm))
            ...
            if w4 > -1:
                inteiwk = int_edge_wire_seg(
                    ei, w4, nk, nl, nm, nn, gnk, gnl, gnm, gnn)
                rhs_fillterm = Io*inteiwk*le[ei_ind]*mu0
                elid_rhs = ei_ind
                elidRHS.append(('RHS', elid_rhs, rhs_fillterm))
            ...
            if w5 > -1:
                inteiwk = int_edge_wire_seg(
                    ei, w5, nk, nl, nm, nn, gnk, gnl, gnm, gnn)
                rhs_fillterm = Io*inteiwk*le[ei_ind]*mu0
                elid_rhs = ei_ind
                elidRHS.append(('RHS', elid_rhs, rhs_fillterm))
            ...
            if w6 > -1:
                inteiwk = int_edge_wire_seg(
                    ei, w6, nk, nl, nm, nn, gnk, gnl, gnm, gnn)
                rhs_fillterm = Io*inteiwk*le[ei_ind]*mu0
                elid_rhs = ei_ind
                elidRHS.append(('RHS', elid_rhs, rhs_fillterm))
...
if __name__ == '__main__':
    from inpt_NextGen_Parallel_FE_EM_wire_source import *
    ...
    bndi1 = np.where((np.abs(nodes[:, 0]-xmin) < 1.e-4)
                     | (np.abs(nodes[:, 0]-xmax) < 1.e-4))[0]
    bndi2 = np.where((np.abs(nodes[:, 1]-ymin) < 1.e-4)
                     | (np.abs(nodes[:, 1]-ymax) < 1.e-4))[0]
    bndi3 = np.where((np.abs(nodes[:, 2]-zmin) < 1.e-4)
                     | (np.abs(nodes[:, 2]-zmax) < 1.e-4))[0]
    ...
    edge_total, edg_tot_inv_i = np.unique(np.concatenate(
        (edge1, edge2, edge3, edge4, edge5, edge6), axis=0), axis=0, return_inverse=True)
...
#     return [rx_pt[0],rx_pt[1],rx_pt[2],np.real(E_rx[0]/nrxtet),np.imag(E_rx[0]/nrxtet),np.real(E_rx[1]/nrxtet),np.imag(E_rx[1]/nrxtet),np.real(E_rx[2]/nrxtet),np.imag(E_rx[2]/nrxtet)]
    ...
    with Pool(processes=job_cpus) as pool:
        tet_info = list(
            pool.map(create_sparse_initial_FE_matrix_vector, tet_indices))

    tic2 = time.time()

    print('Completed tet organization in ', tic2-tic1,
          ' seconds. Used ', job_cpus, ' of ', n_cpu, ' CPUs.')
    ...
    # P and Q have identical non zero element indices
    row_id_A, col_id_A = divmod(el_id_Q, n_in_edge)
    ...
    vect = 'b.mtx'
    b_dense = b_coo.todense().reshape(n_in_edge, 1)
    print('Shape of b Vector is {}'.format(b_dense.shape))
    mmwrite(vect, b_dense)
    ...
    guess = guess_coo.todense().reshape(n_in_edge, 1)
    print('Shape of x Vector is {}'.format(guess.shape))
    start = 'x0.mtx'
    mmwrite(start, guess)
