...
int main(int argc, char* argv[])
{
    ...
    //using vec = gko::matrix::Coo<std::complex<double>>;
    ...
    //using mtx = gko::matrix::Coo<std::complex<double>, ValueType>;
    using mtx = gko::matrix::Coo<ValueType>;
    //using mtxDense = gko::matrix::Dense<std::complex<double>,ValueType>;
    // The gko::solver::Cg is used here, but any other solver class can also be
    // used.
    using cg = gko::solver::Cg<std::complex<double>>;
    ...
    b = gko::as<vec>(b->transpose());
    auto x = gko::read<vec>(std::ifstream("data/x0.mtx"), exec);

    //auto b = gko::read<gko::read<mtxDense>(std::ifstream("../data/b.mtx"), exec);
    std::cout << "Read x0\n";
    //auto x = gko::read<mtxDense>(std::ifstream("../data/x0.mtx"), exec);
    std::cout << "All Read\n";
    x = gko::as<vec>(x->transpose());
    ...
    for (int i = 0; i < x->get_size()[0]; ++i) {
        outputFile << x->get_const_values()[i] << "\n";
    }

    // Close the file
    outputFile.close();

    std::cout << "Solution written to file x.txt successfully." << std::endl;
